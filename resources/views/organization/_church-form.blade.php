...
<form class="grid gap-5" method="POST" action="{{ $action }}" data-dependent-cities data-cities-base-url="{{ url('/organization/states') }}" data-postal-code-lookup-url="{{ url('/organization/postal-codes') }}">
    ...
    <div class="grid gap-5 sm:grid-cols-2">
        <div>
            <label class="ui-label" for="church_postal_code">CEP <span class="font-normal text-text-disabled">(opcional)</span></label>
            <input class="ui-input px-3.5 py-3" id="church_postal_code" name="postal_code" value="{{ old('postal_code', $church?->postal_code) }}" inputmode="numeric" maxlength="9" placeholder="00000-000" aria-describedby="church_postal_code_feedback" autocomplete="postal-code" data-postal-code>
        ...
        <div>
            ...
            <select class="ui-select px-3.5 py-3" id="church_status" name="status">
    ...
    <div class="grid gap-5 sm:grid-cols-2">
        <div>
            <label class="ui-label" for="church_state">Estado <span class="font-normal text-text-disabled">(opcional)</span></label>
            <select class="ui-select px-3.5 py-3" id="church_state" name="state_id" data-state-select>
        ...
        <div>
            <label class="ui-label" for="church_city">Cidade <span class="font-normal text-text-disabled">(opcional)</span></label>
            <select class="ui-select px-3.5 py-3 disabled:bg-surface-muted" id="church_city" name="city_id" data-city-select @disabled(! $selectedStateId)>
    ...
    <div class="grid gap-5 sm:grid-cols-[1fr_8rem]">
        <div>
            <label class="ui-label" for="church_street">Logradouro <span class="font-normal text-text-disabled">(opcional)</span></label>
            <input class="ui-input px-3.5 py-3" id="church_street" name="street" value="{{ old('street', $church?->street) }}" maxlength="255" autocomplete="address-line1" data-street-input>
            @error('street') <p class="mt-2 text-sm text-danger">{{ $message }}</p> @enderror
        </div>
        <div>
            <label class="ui-label" for="church_number">Número <span class="font-normal text-text-disabled">(opcional)</span></label>
            <input class="ui-input px-3.5 py-3" id="church_number" name="number" value="{{ old('number', $church?->number) }}" maxlength="30" placeholder="S/N">
    ...
    <div class="grid gap-5 sm:grid-cols-2">
        <div>
            <label class="ui-label" for="church_neighborhood">Bairro <span class="font-normal text-text-disabled">(opcional)</span></label>
            <input class="ui-input px-3.5 py-3" id="church_neighborhood" name="neighborhood" value="{{ old('neighborhood', $church?->neighborhood) }}" maxlength="255" data-neighborhood-input>
    ...
    <div class="flex flex-col-reverse gap-3 border-t border-border-default pt-5 sm:flex-row sm:justify-end">
        ...
        <button class="ui-button-primary px-5" type="submit">{{ $submitLabel }}</button>
    </div>
</form>
