                    ...
                    <div class="flex items-center gap-2" data-header-user>
                        @if (($availableAccessChurches ?? collect())->isNotEmpty())
                            <form class="hidden md:block" method="POST" action="{{ route('active-church.update') }}">
                                @csrf
                                @method('PATCH')
                                <label class="sr-only" for="active_church_id">Contexto organizacional</label>
                                <select class="ui-select w-auto max-w-[42vw] py-2 text-xs" id="active_church_id" name="church_id" onchange="this.form.submit()" aria-label="Contexto organizacional" data-autosize-select data-select-min-width="7" data-select-max-width="28">
                                    @if (auth()->user()->isGlobalAdministrator())
                                        <option value="__overview__" @selected($activeChurch === null)>Área</option>
                                    @endif
